...
/**
 * A planet of a nearby star. Parameters follow the NASA Exoplanet Archive:
 * masses are m·sin i from radial velocity unless the method pins the true
 * mass, and only published values appear - a missing field means the
 * measurement does not exist, never a guess.
 */
export interface NearStarPlanet {
  name: string;
  status: 'confirmed' | 'candidate';
  /** Minimum mass (m sin i) or true mass where known, Earth masses. */
  massEarth?: number;
  periodDays?: number;
  discovered: number;
  method: string;
...
export interface NearStar {
  ...
  system: string;
  note: string;
  /** Known planets (NASA Exoplanet Archive); absent = none catalogued. */
  planets?: NearStarPlanet[];
}
...
export const NEAR_STARS: NearStar[] = [
  ...
  { id: 'proxima', name: 'Proxima Centauri', raH: 14.495, decDeg: -62.68, distanceLy: 4.25, spectral: 'M5.5V', mag: 11.13, group: 'neighbourhood', note: 'The closest star to the Sun. Hosts Proxima b, an Earth-mass planet in the habitable zone - though flares may scour it.', tempK: 3040, system: 'Third member of the Alpha Centauri triple system; orbits the AB pair every ~550,000 years', planets: [
    { name: 'Proxima b', status: 'confirmed', massEarth: 1.07, periodDays: 11.19, discovered: 2016, method: 'radial velocity', note: 'In the habitable zone; likely tidally locked, and scoured by flares.' },
    { name: 'Proxima d', status: 'confirmed', massEarth: 0.26, periodDays: 5.12, discovered: 2022, method: 'radial velocity', note: 'One of the lightest planets ever found by radial velocity.' },
    { name: 'Proxima c', status: 'candidate', massEarth: 7, periodDays: 1928, discovered: 2019, method: 'radial velocity', note: 'A cold super-Earth candidate; the signal remains debated.' },
  ] },
  ...
  { id: 'lalande21185', name: 'Lalande 21185', raH: 11.06, decDeg: 35.97, distanceLy: 8.31, spectral: 'M2V', mag: 7.52, group: 'neighbourhood', note: 'The brightest red dwarf in the northern sky, still below naked-eye visibility.', tempK: 3550, system: 'Single red dwarf with two confirmed planets', planets: [
    { name: 'Lalande 21185 b', status: 'confirmed', massEarth: 2.7, periodDays: 12.95, discovered: 2017, method: 'radial velocity' },
    { name: 'Lalande 21185 c', status: 'confirmed', massEarth: 14, periodDays: 2946, discovered: 2021, method: 'radial velocity' },
  ] },
  { id: 'sirius-ab', name: 'Sirius A+B', raH: 6.75, decDeg: -16.72, distanceLy: 8.6, spectral: 'A1V + DA2', mag: -1.46, group: 'neighbourhood', note: 'The brightest star in our night sky, with a white-dwarf companion - the burnt-out core of a star that died 120 million years ago.', tempK: 9940, system: 'Binary: A-type main-sequence star + white dwarf (Sirius B, 50-year orbit)' },
  ...
  { id: 'epsiloneri', name: 'Epsilon Eridani', raH: 3.55, decDeg: -9.46, distanceLy: 10.48, spectral: 'K2V', mag: 3.73, group: 'neighbourhood', note: 'A young orange star with a dusty debris disk and a Jupiter-like planet - a favourite of science fiction.', tempK: 5080, system: 'Single star with a debris disk and one confirmed giant planet', planets: [
    { name: 'Epsilon Eridani b', status: 'confirmed', massEarth: 210, periodDays: 2770, discovered: 2000, method: 'radial velocity + astrometry', note: 'A Jupiter-like giant, its mass pinned by Hubble astrometry in 2023.' },
  ] },
  { id: 'lacaille9352', name: 'Lacaille 9352', raH: 23.1, decDeg: -35.85, distanceLy: 10.72, spectral: 'M0.5V', mag: 7.34, group: 'neighbourhood', note: 'A southern red dwarf with two confirmed super-Earths.', tempK: 3630, system: 'Single red dwarf with two confirmed super-Earths', planets: [
    { name: 'GJ 887 b', status: 'confirmed', massEarth: 4.2, periodDays: 9.26, discovered: 2020, method: 'radial velocity' },
    { name: 'GJ 887 c', status: 'confirmed', massEarth: 7.6, periodDays: 21.8, discovered: 2020, method: 'radial velocity' },
  ] },
  { id: 'ross128', name: 'Ross 128', raH: 11.79, decDeg: 0.8, distanceLy: 11.01, spectral: 'M4V', mag: 11.13, group: 'neighbourhood', note: 'An unusually quiet red dwarf hosting Ross 128 b, a temperate Earth-sized planet.', tempK: 3190, system: 'Single red dwarf with one temperate Earth-sized planet', planets: [
    { name: 'Ross 128 b', status: 'confirmed', massEarth: 1.4, periodDays: 9.87, discovered: 2017, method: 'radial velocity', note: 'A temperate Earth-sized world around an unusually quiet red dwarf.' },
  ] },
  ...
  { id: 'groombridge34', name: 'Groombridge 34 A+B', raH: 0.31, decDeg: 44.02, distanceLy: 11.62, spectral: 'M1.5V + M3.5V', mag: 8.08, group: 'neighbourhood', note: 'A red-dwarf pair in Andromeda; the primary hosts a super-Earth.', tempK: 3600, system: 'Red-dwarf binary; the primary hosts at least one confirmed planet', planets: [
    { name: 'GJ 15A b', status: 'confirmed', massEarth: 3.0, periodDays: 11.44, discovered: 2014, method: 'radial velocity' },
  ] },
  { id: 'epsilonindi', name: 'Epsilon Indi', raH: 22.06, decDeg: -56.78, distanceLy: 11.87, spectral: 'K5V', mag: 4.83, group: 'neighbourhood', note: 'Hosts the coldest directly-imaged giant planet (JWST, 2024) plus a pair of brown dwarfs.', tempK: 4630, system: 'K dwarf + a binary pair of brown dwarfs; one directly-imaged giant planet', planets: [
    { name: 'Epsilon Indi Ab', status: 'confirmed', massEarth: 1900, discovered: 2024, method: 'direct imaging (JWST)', note: 'The coldest giant planet ever imaged (~0 °C cloud tops); its decades-long orbit is still poorly constrained.' },
  ] },
  { id: 'tauceti', name: 'Tau Ceti', raH: 1.735, decDeg: -15.94, distanceLy: 11.91, spectral: 'G8V', mag: 3.5, group: 'neighbourhood', note: 'The nearest single Sun-like star, with four candidate super-Earths - long a first stop in SETI searches.', tempK: 5340, system: 'Single Sun-like star with four candidate super-Earths', planets: [
    { name: 'Tau Ceti g', status: 'candidate', massEarth: 1.75, periodDays: 20, discovered: 2017, method: 'radial velocity' },
    { name: 'Tau Ceti h', status: 'candidate', massEarth: 1.83, periodDays: 49.4, discovered: 2017, method: 'radial velocity' },
    { name: 'Tau Ceti e', status: 'candidate', massEarth: 3.93, periodDays: 163, discovered: 2012, method: 'radial velocity', note: 'Near the inner edge of the habitable zone.' },
    { name: 'Tau Ceti f', status: 'candidate', massEarth: 3.93, periodDays: 636, discovered: 2012, method: 'radial velocity', note: 'Near the outer edge of the habitable zone.' },
  ] },
  ...
  { id: 'pollux', name: 'Pollux', raH: 7.76, decDeg: 28.03, distanceLy: 33.8, spectral: 'K0III', mag: 1.14, group: 'bright', note: 'The nearest giant star, with a confirmed exoplanet.', tempK: 4670, system: 'Single orange giant with one confirmed planet', planets: [
    { name: 'Pollux b (Thestias)', status: 'confirmed', massEarth: 730, periodDays: 590, discovered: 2006, method: 'radial velocity', note: 'A giant planet around a giant star - visible to your naked eye tonight.' },
  ] },
