...
/** Largest and mean difference between two captures inside a given mask. */
function diff(aName, bName, mask, count) {
  ...
  return { max, mean: +(sum / Math.max(count, 1)).toFixed(3), worst };
}
 ...
 */
async function pair(name, bodyId, prepare) {
  await prepare();
  await setSky(true);
  ...
  if (count < 200) {
    ...
    return { pixels: count, max: -1, control: -1 };
  }
  ...
  console.log(
    `  ${name.padEnd(22)} body px=${String(count).padStart(6)}` +
      `  max Δ=${String(b.max).padStart(3)}  mean Δ=${String(b.mean).padStart(6)}` +
      `  (control ${ctl.max})`,
  );
  return { pixels: count, max: b.max, mean: b.mean, control: ctl.max };
}
 ...
 */
const clean = (b) => b.max <= b.control + 4 && b.mean < 0.5;

for (const [id, b] of results) {
  check(
    `the sky does not composite over ${id}`,
    clean(b),
    `max channel-sum difference inside the disc = ${b.max} over ${b.pixels} px, mean ${b.mean} (noise floor ${b.control})`,
  );
}
...
for (const [id, b] of cockpit) {
  check(
    ...
    clean(b),
    `max difference = ${b.max} over ${b.pixels} px (noise floor ${b.control})`,
  );
}
...

{
  ...
  console.log(`  sc-hull                hull px=${String(count).padStart(6)}  max Δ=${String(b.max).padStart(3)}  mean Δ=${b.mean}`);
  check(
    'the sky does not composite over the cockpit frame',
    count > 5000 && b.max <= 6 && b.mean < 0.5,
    `max difference = ${b.max} over ${count} structural px, mean ${b.mean}`,
  );
}
...
for (const [id, b] of worstCase) {
  check(
    ...
    clean(b),
    `max difference = ${b.max} over ${b.pixels} px, mean ${b.mean} (noise floor ${b.control})`,
  );
}
